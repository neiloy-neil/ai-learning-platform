...
import { getConceptsForRevision } from '@/lib/revision-scheduler';
...
export async function GET(_req: Request, context: { params: Promise<{ studentId: string }> }) {
    try {
        ...
        const revisionSuggestions = getConceptsForRevision(masteryProfile, allAttempts, allConcepts);

        return NextResponse.json(revisionSuggestions);
    } catch (error) {
        console.error("[REVISION_GET]", error);

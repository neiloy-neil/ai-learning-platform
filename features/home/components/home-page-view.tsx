...
function TeachingApproachSection() {
  return (
    <section className="px-4 py-16 sm:px-6" id="programs">
      <div className="mx-auto max-w-7xl">
        <SectionHeading align="center" description="Our research-backed methodology combines adaptive technology with proven pedagogical principles to deliver measurable learning outcomes." eyebrow="Teaching Approach" title="A smarter way to learn, backed by science and AI" />
        <div className="mt-10 grid gap-6 md:grid-cols-2">
          {teachingApproach.map((approach, index) => {
            const Icon = iconMap[approach.icon];
            return (
              <m.div
                className="group"
                ...
                whileInView={{ opacity: 1, y: 0 }}
              >
                <Card className="h-full rounded-[1.85rem] border-white/10 bg-white/5 p-6 backdrop-blur-2xl transition-all duration-300 group-hover:border-cyan-300/30 group-hover:bg-white/10">
                  <div className="flex h-14 w-14 items-center justify-center rounded-[1.25rem] bg-gradient-to-br from-violet-500/30 via-fuchsia-500/20 to-cyan-400/20 text-cyan-100 shadow-[0_16px_40px_-24px_rgba(34,211,238,0.9)]">
                    <Icon className="size-6" />
                  </div>
                  <h3 className="mt-6 text-xl font-semibold text-white">{approach.title}</h3>
                  <p className="mt-3 text-sm leading-7 text-slate-300">{approach.description}</p>
...
function ProgramsSection() {
  return (
    <section className="px-4 py-16 sm:px-6" id="programs-pricing">
      <div className="mx-auto max-w-7xl">
        <SectionHeading align="center" description="From free diagnostic assessments to comprehensive school partnerships, we have a program for every learning need." eyebrow="Programs & Pricing" title="Choose the right learning path for your goals" />
        <div className="mt-10 grid gap-5 lg:grid-cols-3">
          {programs.map((plan) => (
            <m.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
            >
              <Card className={cn("h-full rounded-[1.85rem] border-white/10 bg-white/5 p-6 backdrop-blur-2xl", plan.featured && "border-cyan-300/30 bg-cyan-400/10 shadow-[0_28px_90px_-44px_rgba(34,211,238,0.95)]")}>
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm font-semibold text-white">{plan.name}</p>
                    <p className="mt-2 text-4xl font-semibold tracking-tight text-white">{plan.price}</p>
                    <p className="mt-1 text-xs text-slate-400">{plan.duration}</p>
                  </div>
                  {plan.featured ? <Badge className="border-cyan-300/25 bg-cyan-300/10 text-cyan-100" variant="outline">Most Popular</Badge> : null}
                </div>
                <p className="mt-4 text-sm leading-7 text-slate-300">{plan.description}</p>
                <div className="mt-6 space-y-3">
                  {plan.features.map((feature) => (
                    <div className="flex items-center gap-3 text-sm text-slate-200" key={feature}>
                      <CheckCircle2 className="size-4 text-cyan-300" />
                      <span>{feature}</span>
                    </div>
                  ))}
                </div>
                <Button asChild className="mt-8 w-full" size="lg" variant={plan.featured ? "primary" : "secondary"}>
                  <Link href={plan.cta === "Book Now" ? "#booking" : plan.cta === "Request Demo" ? "#contact" : appRoutes.auth.login}>
                    {plan.cta}
                  </Link>
                </Button>
              </Card>
...
function SuccessStoriesSection() {
  return (
    <section className="px-4 py-16 sm:px-6">
      <div className="mx-auto max-w-7xl">
        <SectionHeading align="center" description="Real results from real students and parents who transformed their learning journey with our platform." eyebrow="Success Stories" title="Measurable outcomes that speak for themselves" />
        <div className="mt-10 grid gap-5 md:grid-cols-2">
          {successStories.map((story, index) => (
            <m.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
            >
              <Card className="h-full rounded-[1.85rem] border-white/10 bg-white/5 p-6 backdrop-blur-2xl">
                <div className="flex items-start gap-4">
                  <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-500/20 to-cyan-400/20 text-4xl">
                    {story.image}
                  </div>
                ...
                <p className="mt-6 text-base leading-8 text-slate-200">&ldquo;{story.story}&rdquo;</p>
                <div className="mt-6 rounded-2xl border border-emerald-300/20 bg-emerald-400/10 px-4 py-3">
                  <div className="flex items-center gap-3">
                    <Trophy className="size-5 text-emerald-300" />
                    <p className="text-sm font-semibold text-emerald-100">{story.result}</p>
                  </div>
                </div>
              </Card>
            </m.div>
...
function BookingSection() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedDate || !selectedTime || !formData.studentName || !formData.parentEmail) {
      return;
    }
    setSubmitted(true);
  };
  ...
  if (submitted) {
    return (
      <section className="px-4 py-16 sm:px-6" id="booking">
        <div className="mx-auto max-w-3xl">
          <Card className="rounded-[2rem] border-emerald-300/20 bg-emerald-400/10 p-8 text-center backdrop-blur-2xl">
            ...
            <h3 className="mt-6 text-2xl font-semibold text-white">Booking Confirmed!</h3>
            <p className="mt-3 text-slate-300">
              Your diagnostic assessment is scheduled for <span className="font-semibold text-white">{new Date(selectedDate).toLocaleDateString()}</span> at <span className="font-semibold text-white">{selectedTime}</span>.
            </p>
            <p className="mt-2 text-sm text-slate-400">
              A confirmation email has been sent to {formData.parentEmail} with preparation instructions.
            </p>
  ...
  return (
    <section className="px-4 py-16 sm:px-6" id="booking">
      <div className="mx-auto max-w-7xl">
        <SectionHeading align="center" description="Book a free 60-minute diagnostic assessment to identify learning gaps and receive a personalized improvement plan." eyebrow="Free Diagnostic" title="Book your assessment today" />
        
        <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_1.2fr]">
          <Card className="rounded-[2rem] border-white/10 bg-white/5 p-6 backdrop-blur-2xl">
            <h3 className="text-xl font-semibold text-white">Select Date & Time</h3>
            <p className="mt-2 text-sm text-slate-400">Choose from available slots over the next 2 weeks</p>
            
            <div className="mt-6">
              <label className="text-sm font-semibold text-white">Available Dates</label>
                ...
                {availableDates.slice(0, 12).map((date) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                    >
                      <p className="text-xs text-slate-400">{date.toLocaleString("en", { month: "short" })}</p>
                      <p className="text-lg font-semibold">{date.getDate()}</p>
                      <p className="text-[10px] text-slate-500">{date.toLocaleString("en", { weekday: "short" })}</p>
                    </button>
                  );
                })}
              </div>
            ...
            {selectedDate && (
              <div className="mt-6">
                <label className="text-sm font-semibold text-white">Available Times</label>
            ...
            <div className="mt-6 flex items-center gap-3 rounded-2xl border border-violet-300/20 bg-violet-400/10 p-4">
              <Clock className="size-5 text-violet-300" />
              <div>
                <p className="text-sm font-semibold text-white">60-Minute Session</p>
                <p className="text-xs text-slate-400">Comprehensive assessment + consultation</p>
              </div>
            </div>
          </Card>
          ...
          <Card className="rounded-[2rem] border-white/10 bg-white/5 p-6 backdrop-blur-2xl">
            ...
            <p className="mt-2 text-sm text-slate-400">Fill in the details to complete your booking</p>
            
            <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
              <div>
                <label className="mb-2 block text-sm font-semibold text-white">Student Name *</label>
                <input
                  className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-slate-500 focus:border-cyan-300/50 focus:outline-none focus:ring-2 focus:ring-cyan-300/20"
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-2 block text-sm font-semibold text-white">Parent Email *</label>
                  <input
                    className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-slate-500 focus:border-cyan-300/50 focus:outline-none focus:ring-2 focus:ring-cyan-300/20"
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-2 block text-sm font-semibold text-white">Grade Level</label>
                  <select
                    className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white focus:border-cyan-300/50 focus:outline-none focus:ring-2 focus:ring-cyan-300/20"
                    value={formData.grade}
                    onChange={(e) => setFormData({ ...formData, grade: e.target.value })}
                  >
                    <option className="bg-slate-900" value="">Select grade</option>
                    {Array.from({ length: 6 }, (_, i) => i + 7).map((grade) => (
                      <option className="bg-slate-900" key={grade} value={`Grade ${grade}`}>Grade {grade}</option>
                    ))}
                  </select>
                </div>
                <div>
                  ...
                  >
                    <option className="bg-slate-900" value="Mathematics">Mathematics</option>
                    <option className="bg-slate-900" value="Science">Science (Coming Soon)</option>
                    <option className="bg-slate-900" value="English">English (Coming Soon)</option>
                  </select>
                </div>
              </div>
              ...
              <Button
                className="w-full"
                disabled={!selectedDate || !selectedTime || !formData.studentName || !formData.parentEmail || !formData.parentPhone}
                size="lg"
                type="submit"
              >
              ...
              <p className="text-center text-xs text-slate-500">
                Free assessment • No commitment • Instant results
              </p>
            </form>
          </Card>
        </div>
...
function EnquirySection() {
  return (
    <section className="px-4 py-16 sm:px-6" id="enquiry">
      <div className="mx-auto max-w-7xl">
        <SectionHeading
          align="center"
          description="Start your child's learning journey with a free diagnostic assessment. Fill out the form below and we'll contact you within 24 hours."
          eyebrow="Get Started"
          title="Book your free diagnostic assessment"
        />
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1.2fr_1fr]">
          ...
          <div className="space-y-6">
                ...
                {[
                  {
                    ...
                    description: '60-minute evaluation covering mathematics, reading, and reasoning',
                  },
                  {
                    icon: '📊',
                    title: 'Detailed Results',
                    description: 'Identify learning gaps and strengths with our advanced diagnostic tools',
                  },
                  {
                    icon: '📋',
                    title: 'Personalized Plan',
                    description: 'Receive a customized learning roadmap tailored to your child\'s needs',
                  },
                  {
                    icon: '👨‍👩‍👧',
                    title: 'Parent Consultation',
                    description: 'One-on-one discussion with our education specialists',
                  },
                  {
                    icon: '💰',
                    title: '100% Free',
                    description: 'No obligation, no hidden fees - completely complimentary',
                  },
                ].map((item, index) => (
                  <div key={index} className="flex items-start gap-3">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-cyan-400/10 text-2xl">
                      {item.icon}
                    </div>
                    <div>
            ...
            <Card className="rounded-[2rem] border-cyan-300/20 bg-cyan-400/10 p-6 backdrop-blur-2xl">
              <h3 className="text-lg font-semibold text-white">Why Choose PCDC AI?</h3>
              <ul className="mt-3 space-y-2 text-sm text-slate-300">
                <li>✅ AI-powered adaptive learning</li>
                <li>✅ Experienced teachers and tutors</li>
                <li>✅ Kindy to Year 6 specialists</li>
                <li>✅ Proven results and progress tracking</li>
                <li>✅ Flexible learning modes (Centre, Home, AI)</li>
              </ul>
            </Card>
          </div>
        </div>
  ...
  return (
    <section className="px-4 py-16 sm:px-6" id="faq">
      <div className="mx-auto max-w-4xl">
        <SectionHeading align="center" description="Find answers to common questions about our platform, assessments, and learning programs." eyebrow="FAQ" title="Frequently asked questions" />
        
        <div className="mt-10 space-y-3">
          {faqData.map((faq, index) => (
            <Card
              className="overflow-hidden rounded-[1.5rem] border-white/10 bg-white/5 backdrop-blur-2xl transition-all duration-300 hover:border-white/15"
              key={index}
            >
              <button
              ...
              >
                ...
                {openIndex === index ? (
                  <ChevronUp className="size-5 shrink-0 text-cyan-300" />
                ) : (
...
function ContactSection() {
  ...
  if (submitted) {
    return (
      <section className="px-4 py-16 sm:px-6" id="contact">
        <div className="mx-auto max-w-3xl">
          <Card className="rounded-[2rem] border-emerald-300/20 bg-emerald-400/10 p-8 text-center backdrop-blur-2xl">
            <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-emerald-400/20">
              <Mail className="size-10 text-emerald-300" />
            </div>
  ...
  return (
    <section className="px-4 py-16 sm:px-6" id="contact">
      <div className="mx-auto max-w-7xl">
        <SectionHeading align="center" description="Have questions or need support? Our team is here to help you get started." eyebrow="Contact Us" title="Get in touch with our team" />
        
        <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_1.2fr]">
          <div className="space-y-6">
            <Card className="rounded-[2rem] border-white/10 bg-white/5 p-6 backdrop-blur-2xl">
              <h3 className="text-xl font-semibold text-white">Contact Information</h3>
              <p className="mt-2 text-sm text-slate-400">Reach out through any of these channels</p>
              ...
              <div className="mt-6 space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400">hello@pcdc.ai</p>
                    <p className="text-sm text-slate-400">support@pcdc.ai</p>
                  </div>
                </div>
                
                <div className="flex items-start gap-4">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-violet-400/10">
                    <Phone className="size-5 text-violet-300" />
                  </div>
                  <div>
                    <p className="font-semibold text-white">Phone</p>
                    <p className="text-sm text-slate-400">+880 1701-003664</p>
                    <p className="text-xs text-slate-500">Mon-Sat, 9AM-6PM BST</p>
                  </div>
                </div>
                
                <div className="flex items-start gap-4">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-fuchsia-400/10">
                    <MessageSquare className="size-5 text-fuchsia-300" />
                  </div>
                  <div>
                    <p className="font-semibold text-white">Location</p>
                    <p className="text-sm text-slate-400">Dhaka, Bangladesh</p>
                    <p className="text-xs text-slate-500">In-person consultations by appointment</p>
                  </div>
                </div>
              </div>
            </Card>
          ...
          <Card className="rounded-[2rem] border-white/10 bg-white/5 p-6 backdrop-blur-2xl">
            ...
            <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  >
                    ...
                    <option className="bg-slate-900" value="School Partnership">School Partnership</option>
                    <option className="bg-slate-900" value="Technical Support">Technical Support</option>
                    <option className="bg-slate-900" value="Billing">Billing</option>
                  </select>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  className="min-h-[140px] w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-slate-500 focus:border-cyan-300/50 focus:outline-none focus:ring-2 focus:ring-cyan-300/20"
    ...
    <footer className="border-t border-white/10 px-4 py-10 sm:px-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-8 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="grid gap-8 sm:grid-cols-3">
          <div><p className="text-sm font-semibold text-white">Platform</p><div className="mt-3 space-y-2 text-sm text-slate-400"><Link className="block hover:text-white" href="#features">Features</Link><Link className="block hover:text-white" href="#programs">Programs</Link><Link className="block hover:text-white" href="#booking">Book Diagnostic</Link><Link className="block hover:text-white" href="#faq">FAQ</Link></div></div>
          <div><p className="text-sm font-semibold text-white">Contact</p><div className="mt-3 space-y-2 text-sm text-slate-400"><p>hello@pcdc.ai</p><p>+880 1701-003664</p><p>Dhaka, Bangladesh</p></div></div>
